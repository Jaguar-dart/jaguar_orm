...
abstract class _ProductBean implements Bean<Product> {
  ...
  Future<Product> findByCategory(int categoryId,
      {bool preload: false, bool cascade: false}) async {
...
abstract class _CategoryBean implements Bean<Category> {
  ...
  Future<dynamic> insert(Category model, {bool cascade: false}) async {
    ...
    if (cascade) {
      ...
      if (model.products != null) {
        newModel ??= await find(model.id);
        productBean.associateCategory(model.products, newModel);
        await productBean.insert(model.products);
      }
    }
    return retId;
  }
  ...
  Future<dynamic> upsert(Category model, {bool cascade: false}) async {
    ...
    if (cascade) {
      ...
      if (model.products != null) {
        newModel ??= await find(model.id);
        productBean.associateCategory(model.products, newModel);
        await productBean.upsert(model.products);
      }
    }
    return retId;
  }
  ...
  Future<int> update(Category model,
    ...
    if (cascade) {
      ...
      if (model.products != null) {
        ...
        await productBean.update(model.products);
      }
    }
    return ret;
  }
  ...
  Future<List<Category>> preloadAll(List<Category> models,
    ...
    await OneToXHelper.preloadAll<Category, Product>(
        ...
        (Category model, Product child) => model.products = child,
        cascade: cascade);
    return models;
  }
