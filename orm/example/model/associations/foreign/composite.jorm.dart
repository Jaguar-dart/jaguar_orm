  ...
  Future<dynamic> insert(User model,
    ...
    if (cascade) {
      ...
      if (model.address != null) {
        newModel ??= await find(retId, withConn: withConn);
...
// Return if models is empty. If this is not done, all the records will be returned!
  ...
  Future<int> removeByUser(int userId, String userName,
      {Connection withConn}) async {
    final Remove rm =
  ...
  Future<User> fetchUser(Address model, {Connection withConn}) async {
    return userBean.findOneWhere(
        userBean.id.eq(model.userId), userBean.name.eq(model.userName),
        withConn: withConn);
  }
